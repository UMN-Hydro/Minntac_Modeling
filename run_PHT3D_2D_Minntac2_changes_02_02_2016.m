...
flo_file = 'C:\Hydro_Modeling\lec13\test2_1D_3\test.flo'; % 2D, no recharge
...
nstp = 10;  % number of time steps, incr for better numerical performance, decr for faster simulations
...
% -- General properties
tempC = 10.0; % water temperature, geochem rxs are temp-sensitive
por = 0.39;   % porosity
...
Orgcsed_conc_ic = 0.3*por/rho_b;  % [mol/Lw] * por / rho_b = [mol/g] * current value: arbitrary!!!
...
[mob_eq_comp, mob_eq_ic_z, mob_eq_extra_z, min_eq_comp, min_eq_ic_z, catex_comp, catex_ic_z, ...
    ...
    Minntac_InitCond_chem(sim_dir, phrq_exe, use_file_databas, por, tempC);
...
mob_eq_distr_rech(1,1:(216/x_scale),11) = 10
mob_eq_distr_rech(1,(216/x_scale):(400/x_scale),11) = 5
...
for ii = 1: n_mob_eq
    ...
    % recharge 
    mob_eq_const_rech(ii) = mob_eq_ic_z(3,ii);
end
for ii = 1: n_min_eq
    % Rest of domain
...
% imob_kin_ic(:,:,:,ii) = 0; 
imob_kin_ic(:,:,:,ii) = Orgcsed_conc_ic*10;
